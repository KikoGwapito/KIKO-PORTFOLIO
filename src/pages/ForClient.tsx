...
export default function ForClient() {
  ...
  const renderFormContent = (isMobile: boolean = false) => (
    <div className={`p-6 sm:p-10 rounded-[2.5rem] glass border border-zinc-800/50 shadow-2xl relative ${isMobile ? 'overflow-y-auto max-h-[calc(100vh-14rem)]' : 'overflow-hidden'} ${data.reviews.submissionLimit <= 0 ? 'opacity-75 cursor-not-allowed' : ''}`}>
      ...
      {data.reviews.submissionLimit <= 0 && (
      ...
      <form onSubmit={handleSubmit} className={`flex flex-col gap-6 ${data.reviews.submissionLimit <= 0 ? 'pointer-events-none select-none opacity-50' : ''}`}>
        <div className="space-y-6">
              ...
              {[1, 2, 3, 4, 5].map((star) => (
                <Magnetic strength={0.3} key={star}>
                  <button
                    ...
                    disabled={data.reviews.submissionLimit <= 0}
                  >
                    <Star
          ...
          <Magnetic strength={0.15}>
            <button
              type="submit"
              disabled={isSubmitting || data.reviews.submissionLimit <= 0 || isSubmitted}
